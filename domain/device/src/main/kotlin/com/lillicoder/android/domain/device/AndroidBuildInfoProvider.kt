...
class AndroidBuildInfoProvider : BuildInfoProvider {
    ...
    override fun board() = BuildInfo(
        R.string.build_board,
        Build.BOARD
    )
    override fun bootloader() = BuildInfo(
        R.string.build_bootloader,
        Build.BOOTLOADER
    )
    override fun brand() = BuildInfo(
        R.string.build_brand,
        Build.BRAND
    )
    override fun device() = BuildInfo(
        R.string.build_device,
        Build.DEVICE
    )
    override fun display() = BuildInfo(
        R.string.build_display,
        Build.DISPLAY
    )
    override fun fingerprint() = BuildInfo(
        R.string.build_fingerprint,
        Build.FINGERPRINT
    )
    override fun hardware() = BuildInfo(
        R.string.build_hardware,
        Build.HARDWARE
    )
    override fun host() = BuildInfo(
        R.string.build_host,
        Build.HOST
    )
    override fun id() = BuildInfo(
        R.string.build_id,
        Build.ID
    )
    override fun manufacturer() = BuildInfo(
        R.string.build_manufacturer,
        Build.MANUFACTURER
    )
    override fun model() = BuildInfo(
        R.string.build_model,
        Build.MODEL
    )
    override fun product() = BuildInfo(
        R.string.build_product,
        Build.PRODUCT
    )
    override fun radioVersion() = BuildInfo(
        R.string.build_radio_version,
        Build.getRadioVersion() ?: Build.UNKNOWN
    )
    override fun tags() = BuildInfo(
        R.string.build_tags,
        Build.TAGS
    )
    override fun time() = BuildInfo(
        R.string.build_time,
        Build.TIME.toString()
    )
    override fun type() = BuildInfo(
        R.string.build_type,
        Build.TYPE
    )
    override fun user() = BuildInfo(
        R.string.build_user,
        Build.USER
    )
}
